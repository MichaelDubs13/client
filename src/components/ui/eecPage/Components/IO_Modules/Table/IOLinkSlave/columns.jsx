...
const renderInputs = (cell, header, ioModuleGroupIndex, ioModuleIndex, masterPortIndex) => {
  const ioModuleGroups = ioModuleStore((state) => state.ioModuleGroups);  
  const index = {ioModuleGroupIndex:ioModuleGroupIndex, ioModuleIndex:ioModuleIndex, masterPortIndex:masterPortIndex, slavePortIndex:cell.row.id}
      ...
      return <EditableCell
              {...cell}
                ...
                <InputTextItem inputRef={props.inpRef} className="qz__data-table__editable-cell--input" 
                    item={ioModuleGroups[ioModuleGroupIndex].ioModules[ioModuleIndex].ports[masterPortIndex].ports[cell.row.id]} property={header.property} 
      ...
      return <EditableCell
              {...cell}
                ...
                <DropdownItem inputRef={props.inpRef} className="qz__data-table__editable-cell--input" 
                    options={options}  item={ioModuleGroups[ioModuleGroupIndex].ioModules[ioModuleIndex].ports[masterPortIndex].ports[cell.row.id]} property={header.property}  
      ...
      return <EditableCell
              {...cell}
                ...
                <CreateableSelectItem inputRef={props.inpRef} className="qz__data-table__editable-cell--input" 
                    options={header.options}  item={ioModuleGroups[ioModuleGroupIndex].ioModules[ioModuleIndex].ports[masterPortIndex].ports[cell.row.id]} property={header.property}  
                    index={index} onFocus={props.onFocus} onChange={props.onChange}/>
              )}
            />
    case 'label':
      return <EditableCell
              {...cell}
                renderInput={(props) => (
                  <FormLabel>{ioModuleGroups[ioModuleGroupIndex].ioModules[ioModuleIndex].ports[masterPortIndex].ports[cell.row.id][header.property]}</FormLabel>
              )}
            />
  }
}
...
export const useColumns = (ioModuleGroupIndex, ioModuleIndex, masterPortIndex) => {
  ...
  const columns = headers.map(header => {
    return {
      ...
      cell: (cell) => (
          <div>
            {
              renderInputs(cell, header, ioModuleGroupIndex, ioModuleIndex, masterPortIndex)
            }
          </div>
        ),
        meta: {
